...
namespace sonar
{
    ...
    public static class Tools
    {
            ...
            [Description("Page size. Must be greater than 0 and less or equal than 500. Default: 100. Maximum: 500. Example: 20. Optional.")] int? pageSize = 100)
        {
            ...
            var favorites = await client.GetFromJsonAsync<FavoriteProjectsSearchResponse>($"api/favorites/search?p={page}&ps={pageSize}");
        ...
        {
            var response = await client.GetFromJsonAsync<QualityGateResponse>($"api/qualitygates/get_by_project?organization={organization}&project={project}");
            ...
            )
        {
            ...
            var response = await client.GetFromJsonAsync<ProjectStatusResponse>(url);
            return JsonSerializer.Serialize(response);
        }
            ...
            [Description("The component name. Required.")] string component)
        {
            ...
            var response = await client.GetFromJsonAsync<MeasureComponentResponse>($"api/measures/component?component={component}&metricKeys={metrics}");
            return JsonSerializer.Serialize(response);
        }
    }
}